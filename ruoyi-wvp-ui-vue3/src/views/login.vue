...
import {giteeLogin} from "../api/login.js";
import {deleteDevice} from "../api/wvp/device.js";
...
const loginRules = {
  ...
  publicCode: [{required: true, trigger: "change", message: "请输入公众号code"}]
};
  ...
  proxy.$refs.loginRef.validate(valid => {
    if (valid) {
      ...
      if (giteeStar.value === 'true' && loginForm.value.username !== 'admin') {
        proxy.$modal.confirm("访问本系统需检测是否star本项目，是否继续？").then(() => {
...
onMounted((()=>{
  ...
  let giteeCode = route.query && route.query.code
  if(giteeCode === "true"){
    console.log("登录成功")
    getCookie()
    login()
    ...
    proxy.$modal.msgError("登录失败:请先点star再登录");
  }
}))
</script>
